    ...
    def process_lines(self, filename, outfile, dropout=0, num_workers=1):
        ...
        if num_workers == 1:
            ...
            for i in range(num_workers):
                ...
                pool.apply_async(_process_lines, (self, filename,
                                                  tmp.name, dropout, offsets[i], offsets[i + 1]))
            pool.close()
            pool.join()
            for i in range(num_workers):
            ...
            raise ValueError(
                '`num_workers` is expected to be a positive number, but got {}.'.format(num_workers))
...
if __name__ == '__main__':
    ...
    if sys.version_info < (3, 0):
        ...
        if args.num_workers > 1:
            args.num_workers = 1
            warnings.warn(
                "Parallel mode is only supported in Python3. Using 1 processor instead.")
    ...
    if args.input.name == '<stdin>' or args.num_workers == 1:
        if args.num_workers > 1:
            warnings.warn(
                "In parallel mode, the input cannot be STDIN. Using 1 processor instead.")
        for line in args.input:
            args.output.write(bpe.process_line(line, args.dropout))
    else:
        bpe.process_lines(args.input.name, args.output,
                          args.dropout, args.num_workers)
